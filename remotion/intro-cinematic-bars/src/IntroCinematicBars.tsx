...
const BAR_HEIGHT = 216; // 画面的 20%（1080 * 0.2 = 216px）

export const IntroCinematicBars: React.FC = () => {
  const frame = useCurrentFrame();
  const { width } = useVideoConfig();

  // 上方黑边：frame 0_40 translateY(-216) → 0（收合进来）
  const topBarY = interpolate(frame, [0, 40], [-BAR_HEIGHT, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // 下方黑边：frame 0_40 translateY(216) → 0（收合进来）
  const bottomBarY = interpolate(frame, [0, 40], [BAR_HEIGHT, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // frame 150_180：黑边再次展开（退场）
...
export const introCinematicBarsDefaultProps = {}
