...
```text
我有一个用 Remotion 写的滑入字幕组件（文件：SubtitleSlide.tsx），请帮我做以下调整： 1. 把字幕内容改成以下 4 段（修改 SUBTITLES 阵列的 text 栏位）： - 第 1 段：「___」 - 第 2 段：「___」 - 第 3 段：「___」 - 第 4 段：「___」 2. 把各段的左侧边线颜色改成：「___」、「___」、「___」、「___」（修改 color 栏位） 3. 把字体大小从 48 改成「___」 4. 调整滑入速度：把 slideIn 的 stiffness 从 120 改成「___」（数字越大越快） 请保留原本的滑入滑出动画逻辑，只修改我指定的部分，然后把完整的修改后代码给我。
```
