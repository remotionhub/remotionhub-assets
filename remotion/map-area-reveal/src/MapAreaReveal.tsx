...
export interface OffshoreIcon {
  name: string
  x: number
  y: number
}

export interface MapAreaRevealProps {
  title?: string
  subtitle?: string
  regions?: Region[]
  offshoreIcons?: OffshoreIcon[]
...
export function MapAreaReveal({
  ...
  const regionsWithProjected = regions.map((r) => ({
    ...r,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 区域色块（依序揭示） */}
        {regionsWithProjected
          .filter((r) => !r.isOffshore)
          .map((region) => {
            const sp = spring({
            ...
            return (
              <g key={region.id}>
                ...
                <ellipse
                  cx={region.center.x}
                  cy={region.center.y}
                  rx={region.rx * areaScale}
                  ry={region.ry * areaScale}
                ...
                >
                  <circle
                    ...
                    style={{
                      filter:
                        scale > 0.8
                          ? `drop-shadow(0 0 6px ${region.glowColor})`
                          : 'none',
                    }}
                  />
                  <text
        ...
        {/* 外岛图标（左侧小圆圈） */}
        {offshoreIcons.map((icon, i) => {
          const offshoreRegion = regionsWithProjected.find((r) => r.isOffshore)
          const delay = (offshoreRegion?.startFrame ?? 100) + i * 12
          const sp = spring({
            frame: frame - delay,
            fps,
            config: { damping: 16, stiffness: 220 },
            ...
            <g key={icon.name} transform={`translate(${icon.x}, ${icon.y})`}>
              ...
              <circle
                ...
                style={{
                  filter:
                    scale > 0.8
                      ? `drop-shadow(0 0 6px ${offshoreGlow})`
                      : 'none',
                }}
              />
              <circle
                ...
                style={{
                  filter:
                    scale > 0.8
                      ? `drop-shadow(0 0 4px ${offshoreGlow})`
                      : 'none',
                }}
              />
              <text
      ...
      >
        ...
        {regionsWithProjected.map((region) => {
          const sp = spring({
            frame: frame - region.startFrame,
            fps,
            config: { damping: springDamping, stiffness: springStiffness },
...
export const mapAreaRevealDefaultProps: MapAreaRevealProps = {
  title: '台湾五大区域',
  subtitle: '地理区域揭示',
  springDamping: 20,
  springStiffness: 140,
