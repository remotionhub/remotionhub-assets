...
```text
我有一个用 Remotion 写的快拍列组件（文件：StoriesRow.tsx），请帮我做以下调整： 1. 把 STORIES 数组中的用户名改成「___」 2. 把彩虹未读环颜色从 #f09433 → #bc1888 改成「___」 3. 把头像大小从 96px 改成「___」 4. 把每个圆圈的弹入间隔从每 8 帧改成「___」帧 5. 把直播通知出现时间从第 80 帧改成第「___」帧 6. 把背景颜色从 #0f0f0f 改成「___」 请保留原本的动画效果，只修改我指定的部分，然后把完整的修改后代码给我。
```
