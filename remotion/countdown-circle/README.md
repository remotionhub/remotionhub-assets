...
```text
我有一个用 Remotion 写的圆形进度倒数计时器组件（文件：CountdownCircle.tsx），请帮我做以下调整： 1. 把倒数秒数从 10 改成「___」秒（修改顶部 TOTAL-SECONDS 常数） 2. 把主色从青色 #22d3ee 改成「___」 3. 把最后 N 秒警示触发时间从 3 改成「___」秒（修改 isLast3 判断） 4. 把警示颜色从红色 #ef4444 改成「___」 5. 把完成文字从「完成！」改成「___」 请保留圆弧 stroke-dashoffset 动画逻辑与抖动效果，只修改我指定的部分，然后把完整的修改后代码给我。
```
