...
export const LogoNegativeReveal: React.FC = () => {
  ...
  // frame 0_5：扫描线快速扫过（translateX -200 → +200）
  ...
  // frame 0_40：遮罩圆形 scale 3→0（先快后慢，使用 easeOut 效果）
  ...
  // frame 35_65：外围细边框 SVG circle strokeDashoffset 描绘
  const circlePerimeter = 2 * Math.PI * 88; // r=88 的周长
  const circleStrokeOffset = interpolate(frame, [35, 65], [circlePerimeter, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // frame 60_85：品牌名 y 偏移 +20→0，opacity 0→1
  ...
  return (
    ...
    >
      {/* 底层：白色圆形 Logo 本体（直径 160px，中央「N」字） */}
      <div
        style={{
          position: "absolute",
          top: "50%",
      ...
      {/* 遮罩圆形（背景色，scale 3→0 逐渐收缩露出白色 logo） */}
      <div
        style={{
          position: "absolute",
          top: "50%",
...
export const logoNegativeRevealDefaultProps = {}
