...
// 高铁路线 polyline 点
const hsrPoints = HSR_STATIONS.map((s) => `${s.x.toFixed(1)},${s.y.toFixed(1)}`).join(" ");

// 高铁路径总长度
const HSR_PATH_LENGTH = HSR_STATIONS.reduce((total, s, i) => {
  if (i === 0) return total;
  const prev = HSR_STATIONS[i - 1];
  const dx = s.x - prev.x;
  const dy = s.y - prev.y;
...
// 各站点累积距离比例
const stationRatios = HSR_STATIONS.map((s, i) => {
  if (i === 0) return 0;
  let dist = 0;
  for (let j = 1; j <= i; j++) {
    const dx = HSR_STATIONS[j].x - HSR_STATIONS[j - 1].x;
    const dy = HSR_STATIONS[j].y - HSR_STATIONS[j - 1].y;
    dist += Math.sqrt(dx * dx + dy * dy);
  }
  return dist / HSR_PATH_LENGTH;
});

// 右侧进度条的 4 个主要站
const MAJOR_STATIONS = HSR_STATIONS.filter((s) => s.major);
...
export const MapRouteProgress: React.FC = () => {
  ...
  // 列车当前位置插值
  let trainX = HSR_STATIONS[0].x;
  let trainY = HSR_STATIONS[0].y;
  for (let i = 1; i < HSR_STATIONS.length; i++) {
    ...
    if (trainProgress <= segEnd) {
      ...
      trainX = HSR_STATIONS[i - 1].x + t * (HSR_STATIONS[i].x - HSR_STATIONS[i - 1].x);
      trainY = HSR_STATIONS[i - 1].y + t * (HSR_STATIONS[i].y - HSR_STATIONS[i - 1].y);
      break;
    }
    if (i === HSR_STATIONS.length - 1) {
      trainX = HSR_STATIONS[i].x;
      trainY = HSR_STATIONS[i].y;
    }
  }

  // 已完成路线长度（用于 dashoffset）
  const completedLength = trainProgress * HSR_PATH_LENGTH;
    ...
    >
      ...
      >
        ...
        <polyline
          ...
          stroke="url(#hsrGrad)"
          strokeWidth="5"
          strokeDasharray={`${HSR_PATH_LENGTH}`}
          strokeDashoffset={HSR_PATH_LENGTH - completedLength}
          strokeLinecap="round"
          strokeLinejoin="round"
      ...
      >
        ...
        <div style={{ display: "flex", gap: 20 }}>
          ...
          {/* 站点列表 */}
          <div style={{ flex: 1 }}>
            {MAJOR_STATIONS.map((station, i) => {
              const stationIdx = HSR_STATIONS.findIndex((s) => s.name === station.name);
              ...
              return (
                <div
                  key={station.name}
                  style={{
                    marginBottom: i < MAJOR_STATIONS.length - 1 ? 44 : 0,
...
export const mapRouteProgressDefaultProps = {}
