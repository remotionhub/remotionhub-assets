...
function getVisiblePoints(frame: number): [number, number][] {
  ...
  const numPoints = progress * (PATH_POINTS.length - 1);
  const fullPoints = Math.floor(numPoints);
  const frac = numPoints - fullPoints;
  const visible: [number, number][] = PATH_POINTS.slice(0, fullPoints + 1);
  if (frac > 0 && fullPoints < PATH_POINTS.length - 1) {
    const p0 = PATH_POINTS[fullPoints];
    const p1 = PATH_POINTS[fullPoints + 1];
...
export const ytZeroRevenueDefaultProps = {}
