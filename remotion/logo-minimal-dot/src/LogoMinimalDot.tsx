...
export const LogoMinimalDot: React.FC = () => {
  ...
  // frame 0_30：中央小圆点 scale(0)→scale(1) spring
  ...
  // frame 25_60：圆点直径从 12→120px（spring，轻微 overshoot）
  ...
  // 最终视觉直径（phase 1 先把点缩放出来，phase 2 再展开）
  const currentDiameter = frame < 25 ? 12 * dotInitScale : dotDiameter;

  // frame 55_80：icon 内部斜线 opacity 0→1
  const iconLineOpacity = interpolate(frame, [55, 80], [0, 1], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // frame 70_95：品牌名「Dot Studio」淡入 + translateX
  ...
  // frame 88_108：品牌名下方细横线展开（clip-path）
  const lineClip = interpolate(frame, [88, 108], [100, 0], {
    extrapolateLeft: "clamp",
    extrapolateRight: "clamp",
  });

  // frame 102_120：tagline 淡入
...
export const logoMinimalDotDefaultProps = {}
