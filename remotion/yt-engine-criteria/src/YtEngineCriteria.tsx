    ...
    <AbsoluteFill style={{ background: colors.backgroundGradient }}>
      <AbsoluteFill style={{ opacity: fadeOut }}>
        <div style={{ position: "absolute", top: 100, left: 0, right: 0, textAlign: "center", fontSize: 84, fontWeight: 700, color: colors.text, fontFamily: fonts.main, letterSpacing: 4, transform: `translateY(${titleY}px)`, opacity: titleOpacity }}>
          挑选引擎的
          <span style={{ background: `linear-gradient(90deg, ${colors.accent}, ${colors.accentSecondary})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>两个条件</span>
        </div>
        <CriteriaCard number={1} text="完全透过代码生成，" highlight="不需要操作 GUI" y={380} progress={cond1Spring} icon={<div style={{ display: "flex", alignItems: "center", gap: 12 }}><CodeIcon progress={codeIconDraw} size={105} /><NoGuiIcon progress={noGuiDraw} crossProgress={crossDraw} size={105} /></div>} />
...
export const ytEngineCriteriaDefaultProps = {}
