...
```text
我有一个用 Remotion 写的热门话题墙组件（文件：TrendingHashtags.tsx），请帮我做以下调整： 1. 把第 N 个话题的 Hashtag 从「___」改成「___」 2. 把第 N 个话题的类别从「___」改成「___」 3. 把第 N 个话题的帖子数从「___」改成「___」 4. 把第 N 个话题的 hot 状态从 true/false 改成 false/true 5. 调整标题文字 请保留原本的动画效果，只修改我指定的部分，然后把完整的修改后代码给我。
```
