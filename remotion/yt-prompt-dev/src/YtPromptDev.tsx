...
export const PROMPT_DEV_208_DURATION_FRAMES = 180;

export const YtPromptDev: React.FC = () => {
        ...
        <g transform="translate(960, 30)" opacity={codeSpring}>
          <g transform={`scale(${codeSpring})`}>
            ...
            {CODE_LINES.map((line, i) => {
              const lineStart = i / CODE_LINES.length;
              const lineEnd = (i + 1) / CODE_LINES.length;
            ...
            {codeTypeProg < 1 && (
              <rect x={-180} y={64 + Math.floor(codeTypeProg * CODE_LINES.length) * 38} width={10} height={20} rx={1} fill="#4DA3FF" opacity={Math.sin(frame * 0.3) > 0 ? 0.8 : 0} />
            )}
          </g>
        </g>
...
export const ytPromptDevDefaultProps = {}
