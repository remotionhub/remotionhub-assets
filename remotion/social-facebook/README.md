...
```text
我有一个用 Remotion 写的 Facebook 贴文卡片组件（文件：FacebookPost.tsx），请帮我做以下调整： 1. 把发文者姓名从「陈小华」改成「___」 2. 把贴文内容改成「___」 3. 把图片区的渐层从蓝紫色改成「___」 4. 把反应计数上限从 1200 改成「___」 5. 把留言数和分享数从「234 则留言 · 56 次分享」改成「___」 请保留原本的动画效果，只修改我指定的部分，然后把完整的修改后代码给我。
```
