...
export const YtAudioLetsStart: React.FC = () => {
      ...
      <AbsoluteFill style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", transform: `scale(${scaleOut})` }}>
        {frame < 20 && (
          <svg style={{ position: "absolute", left: 0, top: 0, width: 1920, height: 1080, pointerEvents: "none" }} viewBox="0 0 1920 1080" fill="none">
            <circle cx={CENTER_X} cy={CENTER_Y} r={pulseRingRadius} stroke={GREEN} strokeWidth="3" fill="none" opacity={pulseRingOpacity} />
            <circle cx={CENTER_X} cy={CENTER_Y} r={pulseRing2Radius} stroke={BLUE} strokeWidth="2" fill="none" opacity={pulseRing2Opacity} />
          </svg>
        )}
        ...
        {contentVisible && (
          <>
            {frame >= 22 && frame < 55 && (
              <svg style={{ position: "absolute", left: 0, top: 0, width: 1920, height: 1080, pointerEvents: "none" }} viewBox="0 0 1920 1080" fill="none">
                <circle cx={CENTER_X} cy={CENTER_Y} r={impactRingRadius} stroke={GREEN} strokeWidth="2.5" fill="none" opacity={impactRingOpacity} />
              </svg>
            )}
            ...
            {frame >= 30 && (
              <svg style={{ position: "absolute", left: CENTER_X - 200, top: CENTER_Y - 200, width: 400, height: 400, pointerEvents: "none", transform: `rotate(${ringRotation}deg)` }} viewBox="0 0 400 400" fill="none">
            ...
            <div style={{ position: "absolute", left: CENTER_X - 50, top: CENTER_Y - 160 + playFloat, transform: `scale(${playScale})`, opacity: playOpacity }}>
              <PlayIcon size={100} strokeProgress={playStroke} fillOpacity={0.15} glowIntensity={playGlow} />
            </div>

            <div style={{ position: "absolute", left: 0, right: 0, top: CENTER_Y - 20, textAlign: "center", opacity: textOpacity, transform: `scale(${textScale}) translateY(${textY}px)` }}>
            ...
            {revealSparkles.map((sp, i) => {
              ...
              return <Sparkle key={`reveal-${i}`} x={CENTER_X + Math.cos(rad) * expandDist} y={CENTER_Y + Math.sin(rad) * expandDist} size={sp.size} opacity={spOpacity} rotation={elapsed * 5} color={i % 2 === 0 ? GREEN : BLUE} />;
            })}
            ...
            {isHoldPhase && idleSparkles.map((sp, i) => {
              ...
              return <Sparkle key={`idle-${i}`} x={CENTER_X + Math.cos(rad) * sp.dist} y={CENTER_Y + Math.sin(rad) * sp.dist} size={sp.size} opacity={twinkle} rotation={elapsed * 3 + i * 45} color={i % 2 === 0 ? GREEN : BLUE} />;
            })}
          </>
        )}
      </AbsoluteFill>
...
export const ytAudioLetsStartDefaultProps = {}
