...
```text
我有一个用 Remotion 写的留言互动片尾组件（文件：CommentCtaOutro.tsx），请帮我做以下调整： 1. 把主问题文字从「你有什么想法吗？」改成「___」 2. 把副文字从「在下方留言告诉我们吧！」改成「___」 3. 把留言框提示文字从「新增留言…」改成「___」 4. 把大问号颜色从 `#3b82f6` 改成「___」 5. 把背景色从 `#0f172a` 改成「___」 6. 把假留言头像颜色（第一则）从 `#3b82f6` 改成「___」 请保留原本的弹跳、滑入与游标闪烁动画逻辑，只修改我指定的部分，然后把完整的修改后代码给我。
```
