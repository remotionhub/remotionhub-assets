...
```text
我有一个用 Remotion 写的三秒倒计时片头组件（文件：Countdown3Intro.tsx），请帮我做以下调整： 1. 把每个数字的持续帧数从 30 改成「___」 2. 把数字 3 的圆环颜色从 `#ef4444` 改成「___」 3. 把数字 2 的圆环颜色从 `#f59e0b` 改成「___」 4. 把数字 1 的圆环颜色从 `#22c55e` 改成「___」 5. 把「GO」的圆环颜色从 `#3b82f6` 改成「___」 6. 把背景颜色从 `#0f172a` 改成「___」 请保留原本的倒计时与圆环动画逻辑，只修改我指定的部分，然后把完整的修改后代码给我。
```
